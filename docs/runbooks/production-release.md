...
This is the operational release gate. A release manager owns the final decision; the application team supplies the local evidence and the platform/SRE owner supplies deployed-topology evidence. A missing evidence item is a **no-go**, not an implicit pass.
...
The recorded direct-package review identifies `AWSSDK.S3`, `Google.Apis.Auth`, and `Magick.NET-Q8-AnyCPU` as Apache-2.0, and `Otp.NET` plus `Microsoft.ApplicationInsights.AspNetCore` as MIT. Receipt rendering also uses QuestPDF: the release owner must confirm that the configured `OrderReceipt:QuestPdfLicense` category matches the business's current entitlement. Retain and review backend/frontend license exports with each candidate because dependencies and license terms can change. The account email sender uses .NET's built-in SMTP client; no third-party email package was added.
...
| `20260924001622_EnforceWarrantyCoverageSnapshotIdentity` | Keep warranty activation disabled, reconcile any conflicting duplicate entitlement/component snapshots, then rerun the forward migration and concurrent activation smoke. | Do not delete conflicting coverage history automatically or enable activation before the unique index and race tests pass. |
...
5. Rate limits in this application are per process. For more than one API replica, configure an edge/WAF or distributed limiter as the cluster-wide authority. Preserve independent auth, 2FA, checkout, payment, provider-webhook, comment, warranty, catalog, and default quotas; return `429` with `Retry-After`. Do not key on raw email, phone, token, signature, or untrusted forwarding headers. The per-process limiter is defense-in-depth only.
...
5. Drive the same synthetic identity/IP traffic through both replicas and prove the total accepted requests do not exceed the shared edge/distributed quota. Exhausting checkout/payment must not consume the provider-webhook bucket, and restarting one replica must not reset the shared quota. Preserve redacted edge configuration plus `429`/`Retry-After` metrics as evidence.
